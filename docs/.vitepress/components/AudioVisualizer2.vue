...
const freqBins = 256
const maxDepth = 100
const baseSpacingX = 2.8
const baseSpacingZ = 2

// const colorLow = { r: 50, g: 200, b: 255 }
const colorLow = { r: 0, g: 0, b: 0 }
// const colorHigh = { r: 150, g: 0, b: 255 }
const colorHigh = { r: 400, g: 20, b: 20 }
...
function drawVisualizer() {
    ...
    for (let z = maxDepth - 1; z > 0; z--) {
        for (let x = 0; x < freqBins; x++) {
            historyLines[z][x] = historyLines[z - 1][x] * 0.98
        }
    }

    // Обновляем амплитуды
    for (let x = 0; x < freqBins; x++) {
        const baseVal = (dataArray[x] / 255) * 200 + 30
        historyLines[0][x] = baseVal // без синуса, абсолютное значение
    }
    ...
    for (let z = 0; z < maxDepth; z++) {
        ...
        for (let x = 0; x < freqBins - 1; x++) {
            ...
            // Берем абсолютное значение амплитуды
            const amp = Math.abs(historyLines[z][x]) * scale
            const ampNext = Math.abs(historyLines[z][x + 1]) * scale

            const y = yOffset - amp
            const yNext = yOffset - ampNext
